...
open class Client(
    ...
    suspend fun request(
        ...
        return response.body<ApiResponse>().data
    }
...
internal inline fun <reified T> Client.decode(element: JsonElement?): T? {
    return element?.let { json.decodeFromJsonElement<T>(it) }
}

internal fun Client.json(): Json = json
